...
const PropertyCard = ({ property }) => {
  ...
  return (
    <motion.div
      ...
      className="w-full max-w-sm mx-auto bg-white rounded-2xl shadow-md hover:shadow-xl transition-shadow duration-300 border border-gray-100 overflow-hidden flex flex-col h-full group"
    >
      ...
      <Link to={`/property/${property._id}`} className="flex flex-col h-full">
        ...
        <div className="relative h-64 overflow-hidden bg-gray-100">
          ...
          <div className="absolute top-4 right-4 flex flex-col gap-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300 z-20">
            <button
              onClick={(e) => { e.preventDefault(); /* Add Compare Logic */ }}
              className="bg-white/90 p-2 rounded-full text-gray-600 hover:text-red-500 hover:bg-white shadow-sm transition-colors"
              title="Compare"
            >
              <BarChart2 size={18} />
            </button>
            <button
              ...
              className="bg-white/90 p-2 rounded-full text-gray-600 hover:text-blue-500 hover:bg-white shadow-sm transition-colors"
              title="Share"
            >
              <Share2 size={18} />
            </button>
        ...
        <div className="p-5 flex flex-col grow">
          ...
          <div className="flex justify-between items-start mb-2">
            <h3 className="text-lg sm:text-xl font-bold text-gray-900 leading-tight group-hover:text-red-600 transition-colors line-clamp-1">
              {property.title}
            </h3>
          ...
          {/* CTA Button (Visual only, parent Link handles click) */}
          <button className="w-full py-3 rounded-lg border-2 border-red-500 text-red-600 font-bold text-sm uppercase tracking-wide group-hover:bg-red-500 group-hover:text-white transition-all duration-300 flex items-center justify-center gap-2 pointer-events-none">
            View Details
